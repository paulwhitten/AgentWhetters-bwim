...
def _make_openai_client(api_key: str, base_url: str | None = None) -> AsyncOpenAI:
    ...
    When *base_url* is provided (e.g. Azure AI Foundry), skip the
    AZURE_OPENAI_ENDPOINT path so both backends can coexist.
    """
    if not base_url:
        ...
        return AsyncOpenAI(api_key=api_key, timeout=_LLM_TIMEOUT)
    # Azure AI Foundry requires an api-version query parameter.
    api_version = os.getenv("AZURE_AI_FOUNDRY_API_VERSION", "").strip()
    extra = {"default_query": {"api-version": api_version}} if api_version else {}
    logger.info(
        "Creating AsyncOpenAI client: base_url=%s api_key=%s...%s api_version=%s",
        base_url, api_key[:8], api_key[-4:], api_version or "(none)",
    )
    client = AsyncOpenAI(api_key=api_key, base_url=base_url, timeout=_LLM_TIMEOUT, **extra)
    # Log the resolved base_url the SDK will actually use
    logger.info("SDK resolved base_url: %s", client.base_url)
    return client
...
class OpenAIPurpleAgent(AgentExecutor):
    ...
    def __init__(self, debug: bool = False):
        ...
        # Azure OpenAI deployments override the model name, but only when
        # we are actually using the Azure OpenAI endpoint (no base_url).
    ...
    async def _direct_llm_call(self, user_input: str, ctx_id: str) -> str:
        ...
        for attempt in range(_LLM_MAX_RETRIES + 1):
            try:
                t0 = time.monotonic()
                logger.info(
                    "Fallback LLM request: model=%r base_url=%s",
                    api_kwargs["model"], self._client.base_url,
                )
                completion = await self._client.chat.completions.create(**api_kwargs)
                elapsed = time.monotonic() - t0
                _usage = completion.usage
                logger.info(
                    "Fallback LLM call: %.1fs (attempt %d) model=%s tokens=%s/%s",
                    elapsed, attempt + 1, self._model,
