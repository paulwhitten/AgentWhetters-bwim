...
achieves 91.9\% structural accuracy (147 of 160 rounds correct) with a
cumulative score of $+860$.
...
The 13 remaining failures in the best run fall into three broad categories.
First, 4 rounds where existing deterministic fixes (T-shape extension,
each-end caps) did not trigger because the LLM produced step formats the
pattern matchers did not expect. Second, 3 rounds of L-shape errors where
the LLM stacks blocks vertically instead of extending a row horizontally.
Third, 4 rounds of spatial or color errors, including green agent responses
with wrong colors (2 rounds) and axis confusion on spatial instructions
(2 rounds).
The first category is addressable with broader pattern matching. The
remaining categories involve LLM spatial reasoning limits or green agent
non-determinism that are harder to correct without model upgrades or
additional grounding signals. Budget constraints limit further testing.
